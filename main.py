#!/usr/bin/env python3
"""
🛡️ CyberSec BD Bot - Full Featured Ethical Cybersecurity Bot
Features: Facebook Community Gate, News Feed, Daily Tips, Quiz, Password Checker
"""
...
• [OWASP](https://owasp.org) — Web security guide

*🏆 CTF Platforms:*
• [PicoCTF](https://picoctf.org) — Beginners
• [CTFtime](https://ctftime.org) — Event calendar
...
# ─── VERIFIED USERS (in-memory) ───────────────────────────────────────────────
verified_users: set = set()

# ─── NEWS CACHE ───────────────────────────────────────────────────────────────
news_cache: list = []   # সর্বশেষ fetch করা news এখানে থাকবে
...
def fetch_news(limit=4):
    """Fetch latest news from RSS feeds and update cache."""
    global news_cache
    articles = []
    ...
    if articles:
        ...
        news_cache = articles  # cache আপডেট করুন
        logging.info(f"✅ News cache updated: {len(news_cache)} articles")
    return news_cache[:limit] if news_cache else []


async def scheduled_news_fetch(context: ContextTypes.DEFAULT_TYPE):
    """প্রতি ৬ ঘণ্টায় news cache আপডেট করুন।"""
    logging.info("⏰ Scheduled news fetch running...")
    fetch_news()
    logging.info("✅ News cache refreshed successfully.")
...
def check_password_strength(password: str) -> str:
    ...
    else: feedback.append("❌ কমপক্ষে ৮ অক্ষর হওয়া উচিত")

    if len(password) >= 12: score += 1
    else: feedback.append("⚠️ ১২+ অক্ষর হলে আরো শক্তিশালী হবে")

    if re.search(r'[A-Z]', password): score += 1
    else: feedback.append("❌ বড় হাতের অক্ষর (A-Z) নেই")

    if re.search(r'[a-z]', password): score += 1
    else: feedback.append("❌ ছোট হাতের অক্ষর (a-z) নেই")

    if re.search(r'\d', password): score += 1
    else: feedback.append("❌ সংখ্যা (0-9) নেই")

    if re.search(r'[!@#$%^&*(),.?":{}|<>]', password): score += 1
    else: feedback.append("❌ বিশেষ চিহ্ন (!@#$%) নেই")
    ...
    if score <= 2: strength = "🔴 খুবই দুর্বল (Very Weak)"
    elif score == 3: strength = "🟠 দুর্বল (Weak)"
    elif score == 4: strength = "🟡 মাঝারি (Medium)"
    elif score == 5: strength = "🟢 শক্তিশালী (Strong)"
    else: strength = "💚 খুবই শক্তিশালী (Very Strong) ✅"

    bar = "█" * score + "░" * (6 - score)
    result = f"🔐 *Password Strength Checker*\n\n"
    result += f"Strength: {strength}\n"
    result += f"Score: [{bar}] {score}/6\n\n"
    if feedback:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id not in verified_users:
        await update.message.reply_text(
            ...
            "🛡️ *CyberSec BD Bot*-এ আপনাকে স্বাগতম!\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    await update.message.reply_text(
        f"🔐 *স্বাগতম আবার, {user.first_name}!*\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify":
        ...
        await query.edit_message_text(
            ...
            "আপনি এখন CyberSec BD Bot সম্পূর্ণ ব্যবহার করতে পারবেন!\n\n"
            "আমাদের Community-তে সক্রিয় থাকুন এবং Ethical Cybersecurity শিখুন। 💪\n\n"
    ...
    # ── QUIZ ──
        ...
        result = f"🎉 *সঠিক উত্তর!*\n\n{explain}" if chosen == correct else f"❌ *ভুল!* আপনি: {chosen}\n\n{explain}"
        await query.edit_message_text(
    ...
    # ── PASSWORD CHECK ──
        ...
        await query.edit_message_text(
            ...
            "⚠️ _আসল password পাঠাবেন না!\nShুধু test password দিন।_",
            parse_mode="Markdown",
    ...
    # ── ABOUT ──
        ...
        await query.edit_message_text(
            ...
            "👨‍💻 Made with ❤️ for BD Cyber Community",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📘 Facebook Page", url=FB_PAGE)],
                [InlineKeyboardButton("👥 Facebook Group", url=FB_GROUP)],
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_password"):
        ...
        await update.message.reply_text("👋 /start লিখে মেনু খুলুন।", reply_markup=main_menu_keyboard())
...
def main():
    ...
    # Bot চালু হওয়ার সাথে সাথে প্রথমবার news fetch করুন
    fetch_news()

    # প্রতি ৬ ঘণ্টায় news cache আপডেট (6 * 60 * 60 = 21600 সেকেন্ড)
    ...
    print("🚀 BLACK PRO CYBER Bot চালু হয়েছে!")
    print("📰 News cache প্রতি ৬ ঘণ্টায় আপডেট হবে।")
    app.run_polling(drop_pending_updates=True)


if __name__ == "__main__":
    main()    "🎣 Phishing email চেনার উপায়: অপরিচিত লিঙ্কে ক্লিক করবেন না।",
    "🔄 সফটওয়্যার ও OS নিয়মিত আপডেট করুন — security patch জরুরি।",
    "💾 গুরুত্বপূর্ণ ডেটার backup রাখুন — 3-2-1 rule মেনে চলুন।",
    "🔍 অজানা USB drive কম্পিউটারে কখনো সংযুক্ত করবেন না।",
    "🛡️ Antivirus ও Firewall সবসময় চালু রাখুন।",
